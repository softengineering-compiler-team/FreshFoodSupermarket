<template>
    <div class="small">
        <canvas :id="type+'sale'+'chart'" width="800px" height="400px"></canvas>
    </div>
</template>
<script>
...
export default {
  ...
  created:function(){
    console.log(this.type);
     ...
     this.axios
        .get(this.serverUrl+`/query/${this.type}sale`,this.headconfig)
        .then(res => {
          if(res.data.code==0){
            ...
            var ctx2 = document.getElementById(this.type+'salechart');              
            var myChart2 = new Chart(ctx2, this.data);
          }
          else{
